...
describe("scanProjectPath", () => {
  ...
  it("does NOT propose adoptAgentsMd when AGENTS.md is already flockctl-managed", () => {
    const dir = makeDir();
    writeFileSync(join(dir, "AGENTS.md"), "<!-- AUTO-GENERATED by swarmctl -->\n# rules\n");
    const scan = scanProjectPath(dir);
    expect(scan.conflicts.agentsMd.isManaged).toBe(true);
    expect(scan.proposedActions.some((a) => a.kind === "adoptAgentsMd")).toBe(false);
  });
...
describe("applyImportActions — adoptAgentsMd", () => {
  ...
  it("removes AGENTS.md without copying when it was already managed", () => {
    const dir = makeDir();
    writeFileSync(
      join(dir, "AGENTS.md"),
      "<!-- AUTO-GENERATED by swarmctl -->\n# stale\n",
    );
    applyImportActions(dir, [{ kind: "adoptAgentsMd" }]);
    expect(existsSync(join(dir, "AGENTS.md"))).toBe(false);
