...
const MANAGED_AGENTS_HEADER = "<!-- AUTO-GENERATED by swarmctl";
...
function applyAdoptAgentsMd(projectPath: string, backupDir: string): void {
  ...
  // Defensive: if the file is already flockctl-managed, just delete it — the
  // reconciler will regenerate from .flockctl/AGENTS.md.
  if (original.startsWith(MANAGED_AGENTS_HEADER)) {
...
function applyMergeClaudeMd(projectPath: string, backupDir: string): void {
  ...
  // Safety: only merge regular files. Symlinks are handled by the reconciler.
...
function applyImportClaudeSkill(projectPath: string, backupDir: string, name: string): void {
  ...
  if (lst.isSymbolicLink()) return; // reconciler-managed, skip
...
function detectAgentsMd(projectPath: string): ProjectScan["conflicts"]["agentsMd"] {
  ...
  return {
    ...
    isManaged: text.startsWith(MANAGED_AGENTS_HEADER),
  };
}
