...
function EditWorkspaceTemplateDialog({
  template,
  onClose,
}: {
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div className="space-y-2">
            ...
            <Input id="edit-ws-tpl-scope" value="Workspace" disabled />
          </div>
          <div className="space-y-2">
            <Label htmlFor="edit-ws-tpl-name">Name</Label>
            <Input id="edit-ws-tpl-name" value={template.name} disabled />
          </div>
        </div>
        <div className="space-y-2">
          <Label htmlFor="edit-ws-tpl-description">Description</Label>
          <Textarea
            id="edit-ws-tpl-description"
            value={description}
...
export function WorkspaceTemplatesSection({
  ...
  const { data, isLoading } = useTemplates(0, 100, {
    scope: "workspace",
    workspaceId,
  });
    ...
    <div>
      ...
      {!isLoading && templates.length === 0 && (
        <p className="text-sm text-muted-foreground">
          No workspace-scoped templates yet.
        </p>
      )}
