-- Allow `branch_list`, `checkout`, and `branch_delete` as additional values
-- of `git_audit_log.action`.
--
-- Migration 0046 originally pinned the action enum at
-- ('pull','commit','push'); 0048 widened it to add 'log'. This slice adds
-- the three branch-management operations exposed by the new
-- /git-branches, /git-checkout, and /git-branches/:name endpoints, so
-- every branch op can write a forensic row through the same `runGitCommand`
-- envelope the other git surfaces already use.
--
...
-- recreate-and-rename dance, identical in shape to migration 0048:
...
CREATE TABLE git_audit_log_new (
  ...
  CONSTRAINT git_audit_log_action_check
    CHECK (action IN ('pull','commit','push','log','branch_list','checkout','branch_delete')),
