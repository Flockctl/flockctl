-- Allow `log` as a fourth value of `git_audit_log.action`.
--
-- Migration 0046 pinned the action enum at ('pull','commit','push') via a
-- CHECK constraint. The read-only `git log` browser added in this slice
-- needs to write an audit row too — both for forensic parity with the
-- mutating endpoints (operators want to see "who looked at this branch's
-- history when") and so the existing `runGitCommand` envelope can keep
-- being the single funnel for git invocations.
--
...
-- but ONLY after confirming no row has `action='log'`. Forensic data is
...
CREATE TABLE git_audit_log_new (
  ...
  CONSTRAINT git_audit_log_action_check
    CHECK (action IN ('pull','commit','push','log')),
