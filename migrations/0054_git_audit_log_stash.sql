-- Allow `stash_push`, `stash_list`, `stash_pop`, `stash_drop` as additional
-- values of `git_audit_log.action`.
--
-- Migration 0046 originally pinned the action enum at
-- ('pull','commit','push'); 0048 added 'log'; 0050 added the three branch
-- ops ('branch_list','checkout','branch_delete'); 0051 added 'diff'; 0052
-- added 'discard' and 'fetch'; 0053 added 'show'. This slice adds the
-- working-tree-stash
-- surface — `stash_push` (snapshot the working tree onto the stash stack),
-- `stash_list` (read-only enumeration of stash entries — audited so the
-- forensic surface stays symmetric with `branch_list` / `log`),
-- `stash_pop` (apply + drop the topmost or named entry), and `stash_drop`
-- (forget a single entry). All four need to write a forensic row through
-- the same `runGitCommand` envelope as the existing entries.
--
...
-- recreate-and-rename dance, identical in shape to migrations
-- 0048/0050/0051/0052/0053:
...
CREATE TABLE git_audit_log_new (
  ...
  CONSTRAINT git_audit_log_action_check
    CHECK (action IN (
      ...
      'stash_push','stash_list','stash_pop','stash_drop'
    )),
